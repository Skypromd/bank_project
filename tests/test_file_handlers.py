...
class TestFileHandlers(unittest.TestCase):
    ...
    def test_read_csv(self, mock_read_csv):
        ...
        mock_df = pd.DataFrame(
            {"id": [1], "date": ["2023-01-01"], "amount": [100.50], "currency": ["USD"]}
        )
        ...
        self.assertEqual(
            result, expected, "Ошибка: CSV не прочитан корректно как список словарей"
        )
    ...
    def test_read_excel(self, mock_read_excel):
        ...
        mock_df = pd.DataFrame(
            {"id": [2], "date": ["2023-01-02"], "amount": [200.75], "currency": ["EUR"]}
        )
        ...
        self.assertEqual(
            result, expected, "Ошибка: Excel не прочитан корректно как список словарей"
        )
