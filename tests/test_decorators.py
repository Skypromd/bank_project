...
def test_custom_log_filename():
    with patch('logging.basicConfig') as mock_logging_config:
        ...
        mock_logging_config.assert_called_once_with(filename='test.log', level=logging.INFO,
...
def test_log_no_filename():
    with patch('logging.basicConfig') as mock_logging_config:
        ...
        another_function()
        mock_logging_config.assert_called_once_with(stream=sys.stdout, level=logging.INFO,
                                                    format="%(asctime)s - %(message)s")
