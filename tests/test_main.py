...
class TestMain(unittest.TestCase):
    ...
    def test_main_json_executed_rub_search(self):
        ...
        mock_data = [
            {"date": "2019-12-08T12:00:00", "description": "Открытие вклада", "state": "EXECUTED",
             "operationAmount": {"amount": "40542", "currency": {"code": "RUB"}},
             "from": "Счет 1234", "to": "Счет 5678"},
            {"date": "2019-11-12T12:00:00", "description": "Перевод с карты", "state": "EXECUTED",
             "operationAmount": {"amount": "130", "currency": {"code": "USD"}},
             "from": "MasterCard 7771", "to": "Visa 1293"}
        ]
        with patch("src.main.load_json_file", return_value=mock_data), \
             patch("builtins.input", side_effect=generate_input("1", "EXECUTED", "нет", "нет", "да", "Открытие вклада")), \
    ...
    def test_main_csv_canceled_no_sort(self):
        ...
        mock_data = [
            {"date": "2023-01-01T00:00:00", "description": "Пополнение", "state": "CANCELED",
             "operationAmount": {"amount": "100", "currency": {"code": "RUB"}},
             "from": "Счет 1234", "to": "Счет 5678"}
        ]
        with patch("src.main.read_csv_transactions", return_value=mock_data), \
             patch("builtins.input", side_effect=generate_input("2", "CANCELED", "нет", "нет", "нет")), \
             patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            src.main.main()
            output = mock_stdout.getvalue()
            self.assertIn("Всего банковских операций в выборке: 1", output)
    ...
    def test_main_excel_pending_sort_desc_rub(self):
        ...
        mock_data = [
            {"date": "2023-01-02T00:00:00", "description": "Оплата", "state": "PENDING",
             "operationAmount": {"amount": "50", "currency": {"code": "EUR"}},
             "from": "Visa 1234567890123456", "to": "Счет 5678"},
            {"date": "2023-01-01T00:00:00", "description": "Перевод", "state": "PENDING",
             "operationAmount": {"amount": "75", "currency": {"code": "RUB"}},
             "from": "Счет 1234", "to": "Счет 5678"}
        ]
        with patch("src.main.read_excel_transactions", return_value=mock_data), \
             patch("builtins.input", side_effect=generate_input("3", "PENDING", "да", "по убыванию", "да", "нет")), \
             patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            src.main.main()
            output = mock_stdout.getvalue()
            self.assertIn("Всего банковских операций в выборке: 1", output)
    ...
    def test_main_invalid_choice(self):
        ...
        with patch("builtins.input", side_effect=generate_input("4", "")), \
             patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            src.main.main()
            output = mock_stdout.getvalue()
    ...
    def test_main_invalid_status_empty_list(self):
        ...
        mock_data = []
        with patch("src.main.load_json_file", return_value=mock_data), \
             patch("builtins.input", side_effect=generate_input("1", "INVALID", "EXECUTED", "нет", "нет", "нет")), \
             patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            src.main.main()
            output = mock_stdout.getvalue()
    ...
    def test_main_empty_filtered_list_rub(self):
        ...
        mock_data = [
            {"date": "2023-01-01T00:00:00", "description": "Тест", "state": "EXECUTED",
             "operationAmount": {"amount": "100", "currency": {"code": "USD"}},
             "from": "Счет 1234", "to": "Счет 5678"}
        ]
        with patch("src.main.load_json_file", return_value=mock_data), \
             patch("builtins.input", side_effect=generate_input("1", "EXECUTED", "нет", "да", "нет")), \
             patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            src.main.main()
            output = mock_stdout.getvalue()
            self.assertIn("Не найдено ни одной транзакции", output)
    ...
    def test_main_with_sorting(self):
        ...
        mock_data = [
            {"date": "2023-01-02T00:00:00", "description": "Оплата", "state": "EXECUTED",
             "operationAmount": {"amount": "50", "currency": {"code": "EUR"}},
             "from": "Visa 1234567890123456", "to": "Счет 5678"},
            {"date": "2023-01-01T00:00:00", "description": "Перевод", "state": "EXECUTED",
             "operationAmount": {"amount": "75", "currency": {"code": "RUB"}},
             "from": "Счет 1234", "to": "Счет 5678"}
        ]
        with patch("src.main.load_json_file", return_value=mock_data), \
             patch("builtins.input", side_effect=generate_input("1", "EXECUTED", "да", "по возрастанию", "нет", "нет")), \
             patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            src.main.main()
            output = mock_stdout.getvalue()
    ...
    def test_main_with_input_data(self):
        ...
        with patch("builtins.input", side_effect=generate_input("", "EXECUTED", "нет", "нет", "нет")), \
             patch("sys.stdout", new_callable=StringIO) as mock_stdout:
    ...
    def test_main_with_direct_amount(self):
        ...
        mock_data = [
            {"date": "2023-01-01T00:00:00", "description": "Прямая сумма", "state": "EXECUTED",
             "amount": "500", "currency": {"code": "RUB"}, "from": "Счет 1234", "to": "Счет 5678"}
        ]
        with patch("src.main.load_json_file", return_value=mock_data), \
             patch("builtins.input", side_effect=generate_input("1", "EXECUTED", "нет", "нет", "нет")), \
             patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            src.main.main()
            output = mock_stdout.getvalue()
            self.assertIn("Всего банковских операций в выборке: 1", output)
    ...
    def test_main_simple_output(self):
        ...
        mock_data = [
            {"date": "2023-01-01T00:00:00", "description": "Тест вывода", "state": "EXECUTED",
    ...
    def test_main_with_direct_currency(self):
        ...
        mock_data = [
            {"date": "2023-01-01T00:00:00", "description": "Прямая валюта", "state": "EXECUTED",
             "currency": {"code": "USD"}, "from": "Счет 1234", "to": "Счет 5678"}
        ]
        with patch("src.main.load_json_file", return_value=mock_data), \
             patch("builtins.input", side_effect=generate_input("1", "EXECUTED", "нет", "нет", "нет")), \
             patch("sys.stdout", new_callable=StringIO) as mock_stdout:
    ...
    def test_main_with_only_amount(self):
        ...
        mock_data = [
            {"date": "2023-01-01T00:00:00", "description": "Только сумма", "state": "EXECUTED",
             "amount": "300", "from": "Счет 1234", "to": "Счет 5678"}
        ]
        with patch("src.main.load_json_file", return_value=mock_data), \
             patch("builtins.input", side_effect=generate_input("1", "EXECUTED", "нет", "нет", "нет")), \
             patch("sys.stdout", new_callable=StringIO) as mock_stdout:
    ...
    def test_main_json_file_not_found(self):
        ...
        with patch("src.main.load_json_file", side_effect=FileNotFoundError), \
             patch("builtins.input", side_effect=generate_input("1", "")), \
             patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            src.main.main()
            output = mock_stdout.getvalue()
    ...
    def test_main_with_empty_fields(self):
        ...
        mock_data = [
            {"date": "2023-01-01T00:00:00", "description": "", "state": "EXECUTED",
             "operationAmount": {"amount": "100", "currency": {"code": "RUB"}},
             "from": "", "to": ""}
        ]
        with patch("src.main.load_json_file", return_value=mock_data), \
    ...
    def test_main_with_max_description(self):
        ...
        mock_data = [
            {"date": "2023-01-01T00:00:00", "description": long_desc, "state": "EXECUTED",
